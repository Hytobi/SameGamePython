...
class ModeleSame:
    ...
    def supprime_composante(self, num_compo):
        ...
        for j in range(self.__nbcol):
            if self.__nb_elts_compo[num_compo]>=2 :
                n += self.supprime_composante_colonne(j, num_compo)
    ...
    def est_vide(self, i, j):
        '''Méthode qui vérifie si la case est vide a la position (i,j).
           Argument : self --- class --- ModeleSame
                      i --- int --- coordonnée de la ligne
                      j --- int --- coordonnée de la colonne
           Retour : bool'''
    ...
    def supprime_composante_colonne(self, j, num_compo):
        '''Méthode qui supprime les billes de la composante num_compo qui
           se trouve dans la colonne j.
           Arguments : self --- class --- ModeleSame
                       j --- int --- coordonnée de la colonne
                       num_compo --- le numéro de la composante à supprimer
           Retour : case_sup ---int--- nb de case supprimé dans la colonne'''
        liste = []
        case_sup = 0
        for i in range(self.__lig):
            if self.__mat[i][j].composante() == num_compo :
                ...
                liste.append(self.__mat[i][j])

        liste.reverse()
        while len(liste)!= self.__lig:
            liste.append(Case(-1,-1))
        liste.reverse()
                 
        i = self.__lig-1
        while i != -1:
            self.__mat[i][j] = liste[i]
            i-=1
            
        return case_sup
    
    def supprime_colonnes_vides(self):
        '''Méthode qui supprime une colonne lorsqu'elle est vide.
           Arguments : self --- class --- ModeleSame
           Retour : None'''
        n=0
        for colonne in range(self.__nbcol):
            for ligne in range(self.__lig):
                if self.est_vide(ligne,colonne):
                    n+=1
            if n == self.__lig :
                for i in range(self.__lig):
                    del(self.__mat[i][colonne])
                    self.__mat[i].append(Case(-1,-1))
                n=0
            else :
                n =0        
